...
| 能力 | 状态 | 位置 |
|------|------|------|
| 语言类型枚举 | 已具备 | `types/type.go` |
| 运行时值包装 `Value` | 已具备 | `types/value.go` |
| 类型注册表 `TypeRegistry` | 已具备 | `types/registry.go` |
| 内置类型注册 | 已具备 | `types/builtin.go` |
| 操作符绑定 | 已具备 | `types/registry.go` |
| 自定义操作符 | 已具备 | `types/registry.go` |
| 包函数注册 | 规划中 | `types/registry.go` |
| Go 函数反射绑定 | 已具备 | `types/bind.go` |
| 类型构造与拆包 | 已具备 | `types/coerce.go` |
...
| Go 包函数调用 | 规划中 | `{ "Len": [{"pkg": "str"}, {"literal": {"type": "string", "value": "hello"}}] }` |
...
| `return` | 已具备 | 返回程序结果 |
| `break` | 规划中 | 退出当前循环 |
| `continue` | 规划中 | 跳过当前循环剩余语句 |
| `panic` | 规划中 | 映射 Go `panic` 关键字 |
| `routine` | 规划中 | 映射 Go `go` 关键字 |
